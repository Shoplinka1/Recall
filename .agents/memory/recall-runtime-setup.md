...
**How to apply:** Before runtime verification, install from pnpm-lock.yaml, run the existing Drizzle push command for the development schema, then restart the existing Recall API and web workflows. If the app is not registered with the artifact registry, use direct HTTP checks and report that screenshot capture is unavailable rather than creating a replacement artifact.